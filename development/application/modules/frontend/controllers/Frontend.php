...
class Frontend extends MX_Controller
{
    ...
    public function index()
    {
        ...
        $this->load->view('frontend_new', $data);
    }
    ...
    function send()
    {
        ...
        if ($response->success) {
            if ($response->score > 0.8) {
                ...
                $this->session->set_flashdata('feedback', lang('no_sent'));
                redirect('frontend?contact_message=failed#contact_us');
            }
        } else {
            $this->session->set_flashdata('feedback', lang('no_sent'));
            redirect('frontend?contact_message=failed#contact_us');
        ...
        if (!empty($to)) {
            ...
            redirect('frontend?contact_message=success#contact_us');
        } else {
        ...
        if ($trial_version == '1') {
            ...
            if ($gateway == 'PayPal') {
                ...
                if ($response == 'yes') {
                    ...
                    redirect('frontend?#register_hospital');
                }
                ...
                if ($chargeJson['status'] == 'succeeded') {
                    ...
                    $this->session->set_flashdata('feedback', lang('Please_check_card_details'));
                    redirect('frontend?hospital_message=Please_check_card_details#register_hospital');
                }
                ...
                $callback_url = base_url() . 'frontend#register_hospital';
    ...
    public function addNewhospital($data1)
    {
        ...
        if ($this->ion_auth->email_check($email)) {
            ...
            redirect('frontend?hospital_message=this_email_address_is_already_registered#register_hospital');
        } else {
...
For Any Support Please Contact with Phone No: {phone}';
            ...
            if ($gateway != 'Paystack') {
                if ($gateway == 'Pay U Money') {
                    $this->session->set_flashdata('feedback', 'yes');

                    redirect('frontend?hospital_message=new_hospital_created#register_hospital');
                } else {
