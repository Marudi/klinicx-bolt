...
class Patient extends MX_Controller
{
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            if (empty($id)) {     // Adding New Patient
                if ($this->ion_auth->email_check($email)) {
                    ...
                    $dfg = 5;
                    if ($cross_con=='Yes'){
                        $hospital_access=$this->input->post('hospital_access');
                        
                        $access=implode('***',$hospital_access);
                        $data['access']=$access;
                    }
    ...
    function medicalHistory()
    {
        ...
        $patient_det=$this->patient_model->getPatientById($id);
        ...
        if ($patient_hospital_id != $this->session->userdata('hospital_id')) {
            if($patient_det->cross_con=='No'){
               
                redirect('home/permission');
            }else{
               
                $access=explode('***',$patient_det->access);
               
                if(!in_array('All',$access) && !in_array($this->session->userdata('hospital_id'),$access)){
                    redirect('home/permission');
                }
            }
