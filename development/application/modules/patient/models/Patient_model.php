...
class Patient_model extends CI_model {
    ...
    function getPatient() {
        ...
        $query1 = $this->db->get('patient')->result();
        $result=array();
        foreach($query1 as $que){
            if($que->hospital_id == $this->session->userdata('hospital_id')){
                $result[]=$que;
            }else{
                if($que->cross_con=='Yes'){
                    $new=array();
                    $new=explode('***',$que->access);
                    if(in_array('All',$new) or in_array($this->session->userdata('hospital_id'),$new)){
                        $result[]=$que;
                    }
                }
            }
    ...
    function getPatientBySearch($search, $order, $dir) {
        ...
        $query1 = $this->db->select('*')
                        ...
                        ->where("(id LIKE '%" . $search . "%' OR name LIKE '%" . $search . "%' OR id_new LIKE '%" . $search . "%' OR phone LIKE '%" . $search . "%' OR address LIKE '%" . $search . "%')", NULL, FALSE)
                        ->get()->result();
        $result=array();
        foreach($query1 as $que){
            if($que->hospital_id == $this->session->userdata('hospital_id')){
    ...
    function getPatientByLimit($limit, $start, $order, $dir) {
        ...
        $query1 = $this->db->limit($limit, $start)->get('patient')->result();
        // print_r($query1);die();
        $result=array();
        foreach($query1 as $que){
            if($que->hospital_id == $this->session->userdata('hospital_id')){
    ...
    function getPatientInfoId($searchTerm) {
        ...
        foreach ($users as $user) {
            ...
            $data[] = array("id" => $user['id'], "text" => $user['name'] . ' (' . lang('id') . ': ' . $user['id_new'] . '- '.lang('phone'). ': '.$user['phone'].'- '.lang('age').': '.$age[0].' years )');
        }
        return $data;
    }
    ...
    function getPatientinfoWithAddNewOption($searchTerm) {
        ...
        foreach ($users as $user) {
            ...
            $data[] = array("id" => $user['id'], "text" => $user['name'] . ' (' . lang('id') . ': ' . $user['id_new'] . ' - '.lang('phone'). ': '.$user['phone'].' - '.lang('age').': '.$age[0]. ' '.lang('years').')');
        }
        return $data;
    }
