    ...
    function getAutoSMSTemplateList() {
        ...
        if (!empty($data['cases'])) {
            $output = array(
                ...
                "recordsTotal" => count($this->sms_model->getAutoSMSTemplate()),
    ...
    function getManualSMSTemplateList() {
        ...
        foreach ($data['cases'] as $case) {
            ...
            if ($this->ion_auth->in_group(array('admin'))) {
                ...
                $options2 = '<a class="btn btn-soft-danger btn-xs btn_width" title="' . lang('delete') . '" href="sms/deleteManualSMSTemplate?id=' . $case->id . '&redirect=sms/smsTemplate" onclick="return confirm(\'Are you sure you want to delete this item?\');"><i class="fa fa-trash"></i></a>';
            }
            $info[] = array(
                $i,
                $case->name,
