...
For Any Support Please Contact Us At: {phone}';
    ...
    function getPackageForHospitalRegisteration()
    {
        ...
        $modules = ['accountant', 'appointment', 'lab', 'bed', 'department', 'donor', 'finance', 'pharmacy', 'laboratorist', 'medicine', 'nurse', 'patient', 'pharmacist', 'prescription', 'receptionist', 'report', 'notice', 'email', 'sms', 'file', 'payroll', 'attendance', 'leave', 'chat'];
        $package_in_list = explode(",", $data['package']->module);
        $option = '';
        for ($i = 0; $i < count($modules); $i++) {
            if (in_array($modules[$i], $package_in_list)) {
                if ($modules[$i] == 'finance') {
                    $option .= '<div class="col-md-6"><i class="fa fa-check"></i> ' . lang('financial_activities') . '<br></div>';
                } elseif ($modules[$i] == 'lab') {
                    $option .= '<div class="col-md-6"><i class="fa fa-check"></i> ' . lang('lab_tests') . '<br></div>';
                } else {
                    $option .= '<div class="col-md-6"><i class="fa fa-check"></i> ' . lang($modules[$i]) . '<br></div>';
                }
            } else {
                if ($modules[$i] == 'finance') {
                    ...
                    $option .= '<div class="col-md-6"><i class="fa fa-times"></i> ' . lang($modules[$i]) . '<br></div>';
                }
            }
        }
        $data['option'] = $option;
